	...
	if (session) {
		...
		if (r_sdp) {
			...
			if(ss_state == nua_callstate_ready && status == 200){
				switch_channel_set_flag(tech_pvt->channel, CF_LATE_MEDIA);
			}
			...
			if (r_sdp && !sofia_test_flag(tech_pvt, TFLAG_SDP)) {
				...
				if (sofia_test_pflag(profile, PFLAG_3PCC)) {
					if (switch_channel_test_flag(channel, CF_PROXY_MODE) || switch_channel_test_flag(channel, CF_PROXY_MEDIA)) {
						...
						if (contact_params && strcasecmp(contact_params, "transport=tls") == 0  &&  switch_channel_direction(tech_pvt->channel) == SWITCH_CALL_DIRECTION_INBOUND) {
